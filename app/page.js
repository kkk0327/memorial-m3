...
const SCENE_CONFIG = {
  'Panorama01': { 
    ...
    hotspots: [
      { type: 'room', target: 'bong1intro', text: '봉안당 1', pitch: 12, yaw: -55 },
  ...
  'Panorama02': { 
    ...
    hotspots: [
      { type: 'room', target: 'bong1intro', text: '봉안당 1', pitch: 10, yaw: -50 },
  ...
  'Panorama03': { 
    ...
    hotspots: [
      { type: 'room', target: 'bong1intro', text: '봉안당 1', pitch: 10, yaw: -30 },
  ...
  'Panorama04': { 
    ...
    hotspots: [
      { type: 'room', target: 'bong1intro', text: '봉안당 1', pitch: 10, yaw: -35 },
  ...
  'Panorama06': { 
    ...
    hotspots: [
      { type: 'room', target: 'bong1intro', text: '봉안당 1', pitch: 10, yaw: -45 },
...
export default function MemorialApp() {
  ...
  useEffect(() => { setImgError(false); }, [currentScene]);
    ...
    if (activeMenu === 'gallery' && isPannellumLoaded && window.pannellum) {
      ...
      if (!data?.isFlat && viewerRef.current) {
        ...
        pannellumInstance.current = window.pannellum.viewer(viewerRef.current, {
          ...
          hfov: isMobile ? 50 : 120, maxHfov: 120, minHfov: 50,
          autoLoad: true, showControls: false,
          hotSpots: (data.hotspots || []).map(hs => ({
            pitch: hs.pitch, yaw: hs.yaw, cssClass: "custom-hotspot",
            createTooltipFunc: (div) => {
              if (hs.type === 'nav') div.innerHTML = `<div class="road-arrow-3d" style="width:${hs.w||55}px; height:${hs.h||85}px; background-color:${hs.color}; transform: translate(-50%, -50%) rotateX(65deg) rotate(${hs.rotate||'0deg'});"></div>`;
  ...
  return (
    <div className="app-container">
      ...
      {activeMenu === 'main' && (
        <div className="main-viewport">
          ...
          <div className="main-overlay">
            <div className="main-header"><h1 className="main-title">추모관</h1><p className="main-subtitle">영원한 안식, 함께 기억합니다.</p></div>
            <div className="bottom-menu">
              <button onClick={() => { if(hasFlowered) { setToastMessage(["이미 헌화하셨습니다.", "따뜻한 마음 감사합니다."]); setShowToast(true); setTimeout(()=>setShowToast(false), 3000); } else { setHasFlowered(true); setIsFlowering(true); setTimeout(()=>setIsFlowering(false), 2600); } }}><Flower2 color="white" /><span>헌화</span></button>
      ...
      {activeMenu === 'video' && (
        <div className="video-full-viewport"><video src="/videos/mo01.mp4" autoPlay playsInline onEnded={startGallery} className="full-video-element" /><button className="video-exit-button" onClick={startGallery}><X size={32} color="white" /></button></div>
      )}
        ...
        <div className="gallery-full-viewport">
          ...
          {SCENE_CONFIG[currentScene]?.isFlat && (
            <div className="flat-scene-wrapper">
              <img src={SCENE_CONFIG[currentScene].img} className="flat-scene-img" alt="상세뷰" onError={() => setImgError(true)} />
              {currentScene === 'yu' && !imgError && <div className="min-seong-clickbox" onClick={() => setCurrentScene('per')}></div>}
            </div>
          )}

          {!SCENE_CONFIG[currentScene]?.isFlat && currentScene !== 'bong1intro' && (
      ...
      <style jsx global>{`
        ...
        .min-seong-clickbox { position: absolute; top: 30%; left: 40%; width: 20%; height: 30%; cursor: pointer; z-index: 115; background: transparent; }
        ...
        .flower-anim { position: absolute; left: 50%; bottom: 25%; transform: translateX(-50%); z-index: 20; animation: flower-up-desktop 2.6s forwards; }
        .flower-anim img { width: auto; max-width: 150px; }
        @keyframes flower-up-desktop { 0% { bottom: 25%; opacity: 0; } 20% { opacity: 1; } 100% { bottom: 60%; opacity: 0; } }
        @media (max-width: 768px) {
          .toast-center { padding: 18px 30px; white-space: nowrap; font-size: 0.95rem; line-height: 1.5; }
          .flower-anim { bottom: 10%; animation: flower-up-mobile 2.6s forwards; }
          .flower-anim img { width: 120px; }
        }
        @keyframes flower-up-mobile { 0% { bottom: 10%; opacity: 0; } 20% { opacity: 1; } 100% { bottom: 45%; opacity: 0; } }
      `}</style>
    </div>
  );
}